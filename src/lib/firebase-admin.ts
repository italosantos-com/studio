...
// Usar APENAS as variáveis do .env.local
const getServiceAccountFromEnv = () => {
  // Para produção, usar as variáveis do .env.local
  const projectId = process.env.FIREBASE_PROJECT_ID;
  const privateKey = process.env.FIREBASE_PRIVATE_KEY;
  const clientEmail = process.env.FIREBASE_CLIENT_EMAIL;
  
  if (!projectId || !privateKey || !clientEmail) {
    throw new Error('Credenciais de admin incompletas no .env.local para produção');
  }
  
  console.log('[Firebase Admin] 🔑 Usando credenciais de admin do .env.local:', {
    projectId,
    clientEmail,
  ...
  // Primeiro, substituir escapes duplos
  if (processedPrivateKey.includes('\\\\n')) {
    processedPrivateKey = processedPrivateKey.replace(/\\\\n/g, '\n');
  }
  // Depois, substituir escapes simples
  else if (processedPrivateKey.includes('\\n')) {
    processedPrivateKey = processedPrivateKey.replace(/\\n/g, '\n');
  }
  
  console.log('[Firebase Admin] 🔧 Processando chave privada:', {
...
export function initializeFirebaseAdmin() {
  try {
    ...
    // Validar se a chave privada está completa
    if (!serviceAccount.private_key.includes('-----BEGIN PRIVATE KEY-----') || !serviceAccount.private_key.includes('-----END PRIVATE KEY-----')) {
      console.error('[Firebase Admin] ❌ Chave privada incompleta ou inválida');
      console.error('[Firebase Admin] A chave privada deve conter BEGIN e END markers');
      console.error('[Firebase Admin] Chave contém BEGIN:', serviceAccount.private_key.includes('-----BEGIN PRIVATE KEY-----'));
      console.error('[Firebase Admin] Chave contém END:', serviceAccount.private_key.includes('-----END PRIVATE KEY-----'));
      return null;
    }
